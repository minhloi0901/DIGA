    ...
    def restore(self):
        if hasattr(self, 'restore_from'):
            ...
            if 'imagenet' in self.restore_from.split("/")[-1] or "resnet" in self.restore_from.split("/")[-1]:
                ...
                self.load_imagenet_pretrained(saved_state_dict['state_dict'])
            elif 'baseline' in self.restore_from.split("/")[-1] or 'dl' in self.restore_from.split("/")[-1] or 'GTA2Cityscapes_LSGAN' in self.restore_from.split("/")[-1] or 'gta_source_baseline' in self.restore_from.split("/")[-1]:
...
class DIGA(LightningModule):
    ...
    def save_segmentation_image(self, prediction: np.ndarray, name: str, save_dir: str = "/kaggle/working/output/results/bdd_real/masks_1"):
        trainId2color = {
            0: (128, 64, 128),   # road
    ...
    def save_image_wise_mious(self, path: str = "/root/duc-loi/output/results/bdd/image_mious_1.csv"):
        import csv
    ...
    def test_epoch_end(self, outputs: List[Any]):
        ...
        for i in range(len(test_accs)):
            self.log(f"test/acc/dataloaderr_idx_{str(i)}", test_accs[i], on_epoch=True, prog_bar=True)
            miou_16 = self.test_acc[i].compute_iou(type="16")
            miou_13 = self.test_acc[i].compute_iou(type="13")
            self.log(f"test/acc/dataloaderr16_idx_{str(i)}", miou_16, on_epoch=True, prog_bar=True)
            self.log(f"test/acc/dataloaderr13_idx_{str(i)}", miou_13, on_epoch=True, prog_bar=True)

            class_iou = self.test_acc[i].compute_class_iou()
            class_names = self.hparams.dataset_info.class_names
            self.wandb.log_table(
                key=f"test/class_iou/dataloaderr_idx_{str(i)}",
                columns=list(class_names),
                data=[class_iou]
            )
        for test_acc in self.test_acc:
            test_acc.reset()
    ...
    def multi_proto_label(self, feature, outputs, y, class_names, cfg, entropy):
        ...
        # make prediction
        # instance_pred = self.cal_pred_of_proto(feature, self.classifier_running_proto, self.classifier_running_proto_exists_flag)
    ...
    def cal_proto(self, feature, y, outputs, ratio=0.5):
        ...
        """
        num_classes = self.hparams.dataset_info["num_classes"]
        proto = torch.zeros(num_classes, feature.shape[1]).to(self.device)
        with_flag = torch.zeros(num_classes).to(self.device)
      
        feature_flat = feature.permute((0, 2, 3, 1)).reshape(-1, feature.shape[1])
        outputs_flat = outputs.permute((0, 2, 3, 1)).reshape(-1, num_classes)
        ...
        for i in range(num_classes):
            ...
            topk = max(1, int(ratio * num_pixels))
            conf = outputs_flat[mask, i]
            
            # get topk pixels based on conf pixel
            topk_vals, topk_idx = torch.topk(conf, topk, largest=True, sorted=True)
            selected_feat = feature_flat[mask][topk_idx]
            
            proto[i] = selected_feat.mean(dim=0)
